...
class Solution(object):
    def getImportance(self, employees, id):
        ...
        # Store all this information in a map
        for i in employees:
            employeeInfo[i.id] = (i.importance, i.subordinates)

        total = 0

        def calcImportance(employeeInfo, id):
            (importance, subs) = employeeInfo[id]
            total = importance
            if (subs is not None):
                for i in subs:
                    total += calcImportance(employeeInfo, i)

            return total

        return calcImportance(employeeInfo, id)
